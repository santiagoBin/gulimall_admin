  ...
  <div class="mod-config">
    ...
    <el-table
      ...
      >
        <template slot-scope="scope">
          <div  class="block">
            <el-image :src="scope.row.logo" style="height: 100px;width: 100px" fit="fill">
              <div slot="error" class="image-slot" style="height: 100px;width: 100px">
    ...
    <el-dialog title="关联分类" :visible.sync="cateRelationDialogVisible" width="30%">
      ...
      <el-table :data="cateRelationTableData" style="width: 100%">
        <el-table-column prop="catelogId" label="序号"></el-table-column>
        ...
        <el-table-column fixed="right" header-align="center" align="center" label="操作">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="deleteCateRelationHandle(scope.row.catelogId,scope.row.brandId)"
            >移除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteCateRelationHandle(catId, brandId) {
        this.$http({
          url: this.$http.adornUrl("/product/categorybrandrelation/delete"),
          method: "post",
          data: this.$http.adornData({catelogId:catId,brandId:brandId}, false)
        }).then(({ data }) => {
          this.getCateRelation();
        });
      },
